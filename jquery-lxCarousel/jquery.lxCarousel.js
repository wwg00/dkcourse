			...
			var lx = {
				init:function(){
					...
					if(opt.type === 'horizontal'){
                        ...
                        this.$ul.width(opt.width*opt.len).height(opt.height);
                        //通过遍历将所有的li浮动
                        this.$ul.find('li').addClass('horizontal');
                    
                    }else if(opt.type === 'fade'){
                        //li添加定位,将所有图片汇聚到一起
                        this.$ul.find('li').addClass('f_fade');
                        this.$ul.height(opt.height).width(opt.width);
                        //页面初始化时,将除了默认index的其他图片进行隐藏
                        // this.$ul.find('li').eq(opt.index).siblings('li').opacity(0);
                        ...
                        this.$ul.width(opt.width*this.idx).height(opt.height);
                        //通过遍历将所有的li浮动
                        this.$ul.find('li').addClass('horizontal');
                    }
					// 移入移除
				...
				show:function(){
					...
					if(opt.type === 'seamless'){
						if(opt.index>this.idx-2){
							opt.index = 1;
							this.$ul[0].style.left = -opt.index*opt.width + 'px';
						}else if(opt.index<-1){
							opt.index = this.idx-4;
							this.$ul[0].style.left = -(opt.index+2)*opt.width + 'px';
						}
						// 动画
						this.animate(this.$ul[0],{left:-(opt.index+1)*opt.width});
						...
						this.animate(this.$ul[0],{left:-opt.index*opt.width});
						this.$page.children('span').removeClass('active');
						this.$page.children('span').eq(opt.index).addClass('active');
					}
